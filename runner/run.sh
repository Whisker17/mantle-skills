...
Environment:
  ...
  OPENROUTER_BYPASS_PROXY Optional; 1/true forces direct connection, 0/false disables it
  OPENROUTER_HTTP_REFERER Optional attribution header for OpenRouter
  OPENROUTER_TITLE        Optional title header for OpenRouter
  API_RETRY_MAX_ATTEMPTS  Optional, defaults to 3 (set 1 to disable retries)
  API_RETRY_BASE_DELAY_SECONDS Optional, defaults to 1
EOF
}
...
openrouter_should_bypass_proxy() {
  if is_truthy "${OPENROUTER_BYPASS_PROXY:-}"; then
    return 0
  fi
  if is_falsy "${OPENROUTER_BYPASS_PROXY:-}"; then
    return 1
  fi
  has_local_proxy_env
}
...
should_retry_request() {
  ...
  grep -Eiq '(^|[^0-9])429([^0-9]|$)|too many requests|rate[ -]?limit' "$stderr_file"
}

run_curl_with_retry() {
  local max_attempts="${API_RETRY_MAX_ATTEMPTS:-3}"
  local base_delay="${API_RETRY_BASE_DELAY_SECONDS:-1}"
  local attempt=1
  local stderr_file=""
  local output_file=""
  local curl_status=0
  ...
  while true; do
    stderr_file=$(mktemp)
    output_file=$(mktemp)
    if curl "$@" >"$output_file" 2>"$stderr_file"; then
      rm -f "$stderr_file"
      cat "$output_file"
      rm -f "$output_file"
      return 0
    ...
    if (( attempt < max_attempts )) && should_retry_request "$curl_status" "$stderr_file"; then
...
api_request() {
  ...
  case "$provider" in
    ...
    openrouter)
      ...
      if openrouter_should_bypass_proxy; then
        curl_args+=(--noproxy "$(url_host "$base_url")")
      fi
